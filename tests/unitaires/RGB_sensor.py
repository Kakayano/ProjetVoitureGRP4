    ...
    def read_data(self):
        ...
        try :
            ...
            if r is None or v is None or b is None:
                with self._lock:
                    self.__rvb = {"rouge": 0, "vert": 0, "bleu": 0}
                raise ValueError("Erreur : Données manquantes ou invalides pour RGB.")
                
            if not r or not v or not b:
                with self._lock:
                    self.__rvb = {"rouge": r if r else 0, "vert": v if v else 0, "bleu": b if b else 0}
                raise ValueError(f"Avertissement : Une ou plusieurs valeurs RGB sont à 0. Valeurs lues: Rouge={r}, Vert={v}, Bleu={b}.")
            ...
            with self._lock:
                ...
                self.__rvb["vert"] = 0
                self.__rvb["bleu"] = 0
                raise ValueError("Erreur de lecture des données RGB.")
